...
public class Base64 {
	private static final char[] CA = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789_$"
			.toCharArray();
	...
	public final static char[] encodeToChar(byte[] sArr, int sLen, boolean lineSep) {
		// Check special case
		if (sLen<0) sLen = sArr != null ? sArr.length : 0;
		if (sLen == 0)
			return new char[0];
	 ...
	 * all (one line).<br>
	 * + Line separator must be "\r\n", as specified in RFC 5 + The array must
	 ...
	 *            No line separator will be in breach of RFC 5 which specifies
	...
	public final static String encodeToString(byte[] sArr, int sLen, boolean lineSep) {
		...
		String ss=new String(encodeToChar(sArr, sLen, lineSep));
		if (ss.endsWith("==")) return ss.substring(0,ss.length()-2);
		if (ss.endsWith("=")) return ss.substring(0,ss.length()-1);
		return ss;
	}
	...
	public final static String encode(String s) {
		...
		try {
			if (s==null) return s;
			return encodeToString(s.getBytes("UTF-8"), -1, false);
	...
	public static String decode(String s){
		try {
			if (s==null) return s;
			return new String(Base64.decodeFast(s), LightUtil.UTF8);
		} catch (UnsupportedEncodingException e) {
			return s;
		}
	}
